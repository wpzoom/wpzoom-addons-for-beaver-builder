	...
	<div class="fl-builder-panel-content-wrap fl-nanoscroller">
		<div class="fl-builder-panel-content fl-nanoscroller-content">
			<div class="fl-builder-blocks">
				...
				<?php do_action( 'wpzabb_fl_builder_ui_panel_before_presets' ); ?>

				<?php

				/** 
				* Renders WPZABB categorized module templates in the UI panel.
				*
				* @see render_ui_panel_modules_templates()
				*/
				if ( ! $is_module_template && $has_editing_cap ) {

					$wpzabb_module_templates = WPZABB_UI_Panels::wpzabb_templates_data( $module_templates, 'includes' );

					if ( count( $wpzabb_module_templates ) > 0 ) :
						foreach ( $wpzabb_module_templates['categorized'] as $cat ) : 
				...
				<?php do_action( 'wpzabb_fl_builder_ui_panel_after_presets' ); ?>

				<div class="fl-builder-modules-cta">
					<a href="#" onclick="window.open('<?php echo admin_url(); ?>options-general.php?page=fl-builder-settings#wpzabb-template-manager');" target="_blank"><i class="fa fa-external-link-square"></i> <?php echo sprintf( __( 'Note - You can enable, disable and manage %s presets here.', 'wpzabb' ), WPZABB_PREFIX );?></a>
				</div>
				<div class="fl-builder-modules-cta">
