(function($){
	 ...
	 * settings page.
	 *
	 * @class WPZABBBuilderAdminSettings
	 * @since 1.0
	 */
	WPZABBBuilderAdminSettings = {
		...
		_bind: function()
		{
			$('.fl-settings-nav a').on('click', WPZABBBuilderAdminSettings._navClicked);
			$('.fl-override-ms-cb').on('click', WPZABBBuilderAdminSettings._overrideCheckboxClicked);
			$('.fl-module-all-cb').on('click', WPZABBBuilderAdminSettings._moduleAllCheckboxClicked);
			$('.fl-module-cb').on('click', WPZABBBuilderAdminSettings._moduleCheckboxClicked);
			$('input[name=fl-templates-override]').on('keyup click', WPZABBBuilderAdminSettings._templatesOverrideChange);
			$('input[name=fl-upload-icon]').on('click', WPZABBBuilderAdminSettings._showIconUploader);
			$('.fl-delete-icon-set').on('click', WPZABBBuilderAdminSettings._deleteCustomIconSet);
			$('#uninstall-form').on('submit', WPZABBBuilderAdminSettings._uninstallFormSubmit);
		},
		...
		{
			$('.fl-override-ms-cb').each(WPZABBBuilderAdminSettings._initOverride);
		},
		...
		{
			$( '.fl-new-license-form .button' ).on( 'click', WPZABBBuilderAdminSettings._newLicenseButtonClick );
		},
		...
		{
			if(WPZABBBuilderAdminSettings._iconUploader === null) {
				WPZABBBuilderAdminSettings._iconUploader = wp.media({
					title: WPZABBBuilderAdminSettingsStrings.selectFile,
					button: { text: WPZABBBuilderAdminSettingsStrings.selectFile },
					library : { type : 'application/zip' },
					multiple: false
				});
			}
			
			WPZABBBuilderAdminSettings._iconUploader.once('select', $.proxy(WPZABBBuilderAdminSettings._iconFileSelected, this));
			WPZABBBuilderAdminSettings._iconUploader.open();
		},
		...
		{
			var file = WPZABBBuilderAdminSettings._iconUploader.state().get('selection').first().toJSON();
		...
		{
			var result = prompt(WPZABBBuilderAdminSettingsStrings.uninstall.replace(/&quot;/g, '"'), '');
