...
/**
 * WPZABB Cloud Templates initial setup
 *
 * @since 1.2.0.2
 */

if( !class_exists('WPZABB_Cloud_Templates') ) {

	class WPZABB_Cloud_Templates {

		private static $instance;
		private static $cloud_url;
		protected static $wpzabb_filesystem = null;
		...
		public static function get_instance(){
			if ( ! isset( self::$instance ) ) {
				self::$instance = new WPZABB_Cloud_Templates();
			}
		...
		function __construct() {
			...
			add_action( 'wp_ajax_wpzabb_cloud_dat_file_fetch', array( $this, 'fetch_cloud_templates' ) );
		}
		...
		static function reset_cloud_transient() {
			...
			$downloaded_templates = get_site_option( '_wpzabb_cloud_templats', false );
		     ...
		     *
		     * So, used update_site_option() to update network option '_wpzabb_cloud_templats'
		     */
			update_site_option( '_wpzabb_cloud_templats', $cloud_templates, true );
		}
		...
		public static function load_filesystem() {
            if ( self::$wpzabb_filesystem === null ) {
                require_once ABSPATH . '/wp-admin/includes/class-wp-filesystem-base.php';
                require_once ABSPATH . '/wp-admin/includes/class-wp-filesystem-direct.php';
                self::$wpzabb_filesystem = new WP_Filesystem_Direct( array() );
            }
        }
		...
		static function message( $msg ) {
			if( !empty( $msg ) ) {
				if( 'not-found' == $msg ) { ?>
					<div class="wpzabb-cloud-templates-not-found">

						<h3> <?php printf( __( 'Welcome to %s Template Cloud!', 'wpzabb' ), 'WPZOOM Addons Pack' ); ?> </h3>
						<p> <?php printf( __( '%s Template Cloud would allow you to browse through our growing library of 150+ professionally designed templates and download the only ones that you need.', 'wpzabb' ), 'WPZOOM Addons Pack' ); ?> <span class="wpzabb-cloud-process button-primary" data-operation="fetch"> <i class="dashicons dashicons-update " style="display: none; padding: 3px;"></i> <?php _e( "Let's get started", "wpzabb" ); ?> &rarr; </span></p>
		...
		static function template_html( $type = 'page-templates' ) {
			...
			if( is_array( $templates ) && count( $templates ) > 0 ) {
				...
				<div class="wpzabb-templates-showcase-<?php echo $type; ?>">

					<?php if( $type == 'page-templates' ) { ?>

						<ul class="wpzabb-templates-filter">
							<li><a class="active" href="#" data-group="all"><?php _e( 'All', 'wpzabb' ); ?> </a></li>
							<?php
								...
								<?php } ?>
						</ul><!-- #wpzabb-templates-filter -->
					<?php } ?>

					<div id="wpzabb-templates-<?php echo $type; ?>" class="wpzabb-templates-<?php echo $type; ?>">
						...
						foreach( $templates as $template_id => $single_post ) {
							...
							$template_class        = ( $data['status'] == 'true' ) ? 'wpzabb-downloaded' : '';
							...
							$tags = implode('", "', $tags);
							?>
							<div id="<?php echo $data['id']; ?>" data-groups='["<?php echo $tags; ?>"]' class="wpzabb-template-block wpzabb-single-<?php echo $type; ?> <?php echo $template_class; ?>" data-is-downloaded="<?php echo $data['status']; ?>">
								<div class="wpzabb-template">

								    <div class="wpzabb-template-screenshot" data-template-name="<?php echo $data['name']; ?>" data-preview-url="<?php echo $data['preview_url']; ?>" data-template-id='<?php echo $data['id']; ?>' data-template-type='<?php echo $type; ?>'>
										...
										<?php if( $type == 'page-templates' ) { ?>
											...
											<span class="more-details"> <?php _e('Preview', 'wpzabb'); ?> </span>
										<?php } else { ?>
											<h2 class="wpzabb-template-name"> <?php echo $data['name']; ?> </h2>
											<div class="wpzabb-count"><?php echo $data['count']; ?></div>
										<?php } ?>

								    </div>
								    <div class="wpzabb-template-info">
									    <h2 class="wpzabb-template-name"> <?php echo $data['name']; ?> </h2>
									    <div class="wpzabb-template-actions">
									    	<span class="button button-sucess wpzabb-installed-btn">
									        	<i class="dashicons dashicons-yes" style="padding: 3px;"></i>
									        	<span class="msg"> <?php _e('Upgrade', 'wpzabb'); ?> </span>
									       	</span>
									    </div>
								    </div>
								</div>
					...
					</div><!-- #wpzabb-templates-list -->

				</div><!-- #wpzabb-templates -->
				...
				if( isset( $_GET['debug'] ) ) {
					if( count( $templates ) < 1 ) {
						?>
						<h2> <?php _e( 'Templates are disabled from RestAPI.', 'wpzabb' ); ?> </h2>
						<?php
						print_r( $templates );
					}
				}
				...
				WPZABB_Cloud_Templates::message( 'not-found' );
			}
		 ...
		 * @since 1.2.0.2
		 */
		static public function create_local_dir( $dir_name = 'wpzoom-bb-addon-pack' ) {
...
*/
$WPZABB_Cloud_Templates = WPZABB_Cloud_Templates::get_instance();
