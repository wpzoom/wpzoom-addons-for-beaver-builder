...
if( !class_exists( 'WPZABB_Helper' ) ) {
	...
	class WPZABB_Helper
	{
		 ...
		 * @return void
		 */
		static public function wpzabb_font_css( $font ){
			$css = '';
		...
		static public function wpzabb_gradient_css( $gradient ){
			...
			if( isset( $gradient['color_one'] ) && $gradient['color_one'] != '' ){
				$color1 = self::wpzabb_hex2rgba( $gradient['color_one'] );
			}

			if( isset( $gradient['color_two'] ) && $gradient['color_two'] != '' ){
				$color2 = self::wpzabb_hex2rgba( $gradient['color_two'] );
			}
